...
// Write the blended pixel to the output texture
    outputTex[inPixel] = float4(1 * intensity.rgb, 1.0);

// Write the same pixel to the four neighboring pixels in the output texture
    outputTex[inPixel + uint2(1, 0)] = float4(intensity.rgb, 1.0);
    outputTex[inPixel + uint2(0, 1)] = float4(intensity.rgb, 1.0);
    outputTex[inPixel + uint2(1, 1)] = float4(intensity.rgb, 1.0);
